...
function updateTotal() { const subtotal = $$('.line-item').reduce((sum, row) => sum + (Number(row.querySelector('.item-quantity').value) || 0) * (Number(row.querySelector('.item-rate').value) || 0), 0); const tax = subtotal * ((Number($('#taxRate').value) || 0) / 100); $('#formTotal').textContent = money(Math.round((subtotal + tax) * 100)); }
function resetInvoiceForm() { $('#invoiceForm').reset(); $('#lineItems').innerHTML = ''; addLine(); const now = new Date(); $('#issueDate').value = isoDate(now); now.setDate(now.getDate() + 30); $('#dueDate').value = isoDate(now); $('#taxRate').value = 0; updateTotal(); }
...
$('#addLine').addEventListener('click', () => addLine()); $('#taxRate').addEventListener('input', updateTotal);
...
(async () => { resetInvoiceForm(); await Promise.all([loadDashboard(), loadClients(), loadInvoices()]); })().catch(err => toast(err.message));
